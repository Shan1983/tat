<?php
/*
 * Get user-db-config.php
 *
 * @package controller
 * @since 1.0
 */
require_once( 'controller/user-db-config.php' );

/*
 * Get login-control.php
 *
 * @package controller
 * @since 1.0
 */
require_once( 'controller/login-control.php' );

/*
 * Get view-course-instance-activate-db.php
 *
 * @package model
 * @since 1.0
 */
require_once( 'controller/view-course-instance-status-db.php' );

/*
 * Get header.php
 *
 * @package model
 * @since 1.0
 */
require_once( 'model/header.php' );

/*
 * Get menu-back-end.php
 *
 * @package model
 * @since 1.0
 */
require_once( 'model/menu-back-end.php' );

/*
 * Get dashboard-container.php
 *
 * @package model
 * @since 1.0
 */
require_once( 'model/admin-view-course-instance-container.php' );

/*
 * Get footer.php
 * 
 * @package model
 * @since 1.0
 */
require_once( 'model/footer.php' );
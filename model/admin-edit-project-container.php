...
//$formData['Skill_Level'] = $_SESSION['all-skills']; loop through all skills and insert them after creating project
    ...
    if(empty($_POST['req-id'])){
        ...
        $user->tat_update_edit($formData, $pid);
        $success = "The project '" . $formData['Title'] . "' has been successfully edited!";
...
// debug remove all skill/project based session data
//unset($_SESSION['all-skills']);unset($_SESSION['skills']);unset($_SESSION['a']);unset($_SESSION['insert-skills']);unset($_POST);
...
<div id="page-wrapper">
    ...
    <div class="row">
        <?php foreach($user->tat_edit_project($pid) as $project) { ?>
        <div class="col-lg-9">
            <form role="form" method="post">
                <div class="row">
                    <div class="col-lg-9">
                        <div class="form-group">
                            ...
                            <input class="form-control"  name="txt-project-name" placeholder="<?php echo $project['Title']; ?>">
                        </div>
                        <div class="form-group">
                            <label>Project Description</label>
                            <p class="help-block">Provide a general description of the project for the students.</p>
                            <textarea class="form-control" rows="3" placeholder="<?php echo $project['Description']; ?>" name="txt-project-description" ><?php if(isset($_POST['txt-project-description'])){echo $_POST['txt-project-description'];} ?></textarea>
                        </div>
                        <div class="form-group">
                ...
                <div class="row">
                    <div class="col-lg-3">
                        <div class="form-group">
                            ...
                            <input class="form-control" name="txt-min-size" value="<?php if(isset($_POST['txt-min-size'])){echo $_POST['txt-min-size'];} ?>" placeholder="<?php echo $project['Size_Min']; ?>">
                        </div>
                    </div>
                    <div class="col-lg-3">
                        <div class="form-group">
                            <label>Maximum Project Size</label>
                            <input class="form-control" name="txt-max-size" value="<?php if(isset($_POST['txt-max-size'])){echo $_POST['txt-max-size'];} ?>" placeholder="<?php echo $project['Size_Max']; ?>">
                        </div>
                    </div>
                </div>
                <div class="row">
                            ...
                            <div class="panel-body">
                                <div class="table-responsive">
                                    <table class="table table-hover">
                                        ...
                                        <tr>
                                            <?php foreach($user->tat_fetch_skill($pid) as $chk) {
                                                if ($cps['Id'] == $chk['Skill_Id']) {
                                                    $_SESSION['checked'] = "checked"; break;
                                                }else{
                                            ...
                                            <td><input type="checkbox" name="req-id[]" value="<?php echo $cps['Id'] ?>"checked></td>
                ...
                <button type="submit" class="btn btn-outline btn-primary" name="do_create_project">Edit Project</button>
            </form>
        </div>
        <?php break; } ?>
        <!-- /.row (nested) -->
